...
.zoom-overlay-open .zoom-overlay {
  ...
  opacity: 1;
    ...
    <div class="d-flex align-items-center mb-4">
        <form action="{{ route('viewfiltereddatabyblock') }}" method="post" class="flex_div user_upload_form py-4" id="filterblock">
            @csrf
            <div>
                <h5 class="text-white">District Name : {{ $district_name }}</h5>
            </div>
            <div class="d-flex col-6 justify-content-end">
               <h5 class="text-white">Block Name :  </h5>
                <select name="block_code" onchange="searchbyblock(event)" style="width:40%;margin-left:20px">
                    <option disabled selected>Select Block</option>
                    @foreach($blocks as $block)
                    <option value="{{$block->block_id}}" @php if(isset($block_code) && ($block->block_id == $block_code)) { echo 'selected'; } @endphp>{{$block->block_name}}</option>
                    @endforeach
                </select>
            </div>
        </form>
