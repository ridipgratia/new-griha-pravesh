    ...
    <script type="text/javascript">
        ...
        async function getBlocks() {
            ...
            await $.ajax({
                ...
                success: function(result) {
                    $('#block_id').html('<option selected disabled>Select Block</option>');
                    $('#gp_id').html('<option selected disabled>Select Gps</option>');
        ...
        async function getGps() {
            ...
            await $.ajax({
                ...
                success: function(result) {
                    ...
                    if (result.status == 200) {
                        $('#gp_id').html('');
                        var content = '<option selected disabled>Select Gps</option>';
                        for (var i = 0; i < result.gps.length; i++) {
                            content +=
                                `<option value="${result.gps[i].gp_id}" >${result.gps[i].gp_name}</option>`;
                        }
                        $('#gp_id').html(content);
                    } else {
                        Swal.fire(
    ...
    <script>
        + function($) {
            ...
            Zoom.prototype._triggerAnimation = function() {
                ...
                $(this._targetImageWrap).css('transform', 'translate(' + this._translateX + 'px, ' + this._translateY +
                    'px) translateZ(0)')
